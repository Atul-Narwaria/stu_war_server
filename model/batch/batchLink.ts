...
export const getBatchStudents =async (page: number,batchId:string,insID:string) => {
    try{
        ...
        return {
            code: 200, status: "success", message:
                await prisma.batchLink.findMany({
                    select:{
                        id:true,
                        fk_student_id:true,
                        fk_bacth_id:true,
...
export const BatchStudentsSearch = async (page: number, query: string, batchId:any, insID: string) => {
    try {
        const skip = (page - 1) * 10;
        let totalPage = await prisma.batchLink.count({
            where: {
        ...
        totalPage = Math.ceil(totalPage / 10);
        return {
            code: 200, status: "success", message: await prisma.batchLink.findMany({
                select:{
                    id:true,
                    fk_student_id:true,
                    fk_bacth_id:true,
